...
if (searchModal) {
  ...
  const renderSearchResult = ({ url, title, publishedAt, description }) => {
    return `
      <a href="${url}" class="block py-3 border-b hover:bg-gray-100 px-2 dark:text-gray-300 dark:hover:bg-transparent dark:border-gray-600">
        <div class="flex justify-between items-center">
          <h5 class="font-semibold">${title}</h5>
          <small class="text-xs dark:text-gray-400">${publishedAt}</small>
        </div>
